...
function getItemCart(data) {
    for (let i = 0; i < data.length; i++) {
        let dataIdAll = data[i]._id
        for (let Basket in getLocalStorage) {
            if(getLocalStorage[Basket].idProduct === dataIdAll){
                console.log(data)
                    const html = `
                        <article class="cart__item" data-id="${dataIdAll}" data-color="${getLocalStorage[Basket].colorsChoice}">
...
function modifyQuantityItem() {
    ...
    for (let i = 0; i < modifyQuantityItemBtn.length; i++){
        modifyQuantityItemBtn[i].addEventListener("change", (stop) => {
            ...
            getLocalStorage[i].productNumber = newQuantity
...
function deleteItem(){
    ...
    for (let i = 0; i < deleteItemBtn.length; i++){
        deleteItemBtn[i].addEventListener("click", (stop) => {
            stop.preventDefault();
            let idDelete = getLocalStorage[i].idProduct;
